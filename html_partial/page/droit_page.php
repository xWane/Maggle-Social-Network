...
$pos = explode("err=",$pos);
    ...
    $check = $pdo->prepare('SELECT user_id FROM page_member WHERE admin = 1 OR admin = 0 AND page_id = :gid');
...
if($row > 0) {
        ...
        foreach($friendss as $friendss) {
            ...
            $check = $pdo->prepare('SELECT admin FROM page_member WHERE user_id = :id AND page_id = :gid');
