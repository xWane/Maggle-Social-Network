...
$posez = explode("err=",$posez);
    ...
    if(!empty($_FILES['avatar']))
    {
        ...
        if ($profilImg !== "") {
            $add = $pdo->prepare('UPDATE `group` SET group_pic = :imgProfil WHERE group_id = :id');
            $add->execute(array(
            ':imgProfil' => $profilImg,
            ':id' => $posez[1]
            ));
    ...
    if(!empty($_FILES['back']))
    {
        ...
        else 
        {
            ...
            if ($backImg !== "") {
            $add = $pdo->prepare('UPDATE `group` SET group_banner = :bannerProfil WHERE group_id = :id');
            $add->execute(array(
    ...
    if(!empty($_POST['surname'])) {
        ...
        $add = $pdo->prepare('UPDATE `group` SET group_name = :name WHERE group_id = :id');
            $add->execute(array(
            ':name' => $name_mod,
            ':id' => $posez[1]
            ));
    ...
    if(!empty($_POST['bio'])) {
        ...
        $add = $pdo->prepare('UPDATE `group` SET bio_g = :biog WHERE group_id = :id');
            $add->execute(array(
            ':biog' => $bio_mod,
            ':id' => $posez[1]
            ));
    ...
    header("location: groupe.php?reg_err=$posez[1]");
?>